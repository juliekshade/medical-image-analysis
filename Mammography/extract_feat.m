...
function [feat] = extract_feat(mammoimg, pec, breastmask, numscales, dtheta)
    ...
    mammoimg_crop = mammoimg(pec(2,2):max(sum(breastmask,1)),1:max(sum(breastmask,2)))
    breastmask_crop = breastmask(pec(2,2):max(sum(breastmask,1)),1:max(sum(breastmask,2)))
    ...
    wavelength = 2.^(0:n/(numscales-1):n) * wavelengthMin
    ...
    for k = 1:size(orientation,2)
        a = reshape(gabormag(:,:,(k*4)-3:k*4),[r*c,4]);
    ...
    for i = 1:size(orientation,2)
        ...
        k_BW = imbinarize(thisimg);
        k_thresh(:,:,i) = k_BW.*thisimg;
    end
    figure;
